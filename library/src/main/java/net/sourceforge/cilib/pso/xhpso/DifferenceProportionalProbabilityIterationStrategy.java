...
import net.sourceforge.cilib.pso.particle.CrossoverParticleBehavior;
        ...
        algorithm.setTopology(algorithm.getTopology().map(new F<Particle, Particle>() {
			...
			public Particle f(Particle p) {
				if (detectionStrategy.detect(p) && !rigidParticles.get(p.getParticleBehavior()).contains(p)) {
	                ...
	                CrossoverParticleBehavior behavior;
	                if(random.getRandomNumber() < 1.0 / (1 + Math.exp(-beta.getParameter() * (diff / Math.abs(other.getBestFitness().getValue()))))) {
	                	behavior = (CrossoverParticleBehavior) other.getParticleBehavior();
	                    behavior.incrementSelectedCounter();
	                } else {
	                	behavior = (CrossoverParticleBehavior) p.getParticleBehavior();
	                }
