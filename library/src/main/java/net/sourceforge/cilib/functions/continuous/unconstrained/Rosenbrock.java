...
public class Rosenbrock extends ContinuousFunction implements Gradient {
    ...
    public Double df(Vector input, int i) {
        ...
        if (i == 1)
        {
            res = -400.0*input.doubleValueOf(0)*input.doubleValueOf(1)+400.0*Math.pow(input.doubleValueOf(0), 3)+2.0*input.doubleValueOf(0)-2.0;
        }
        else if (i >= 2 && i <= n-1)
        {
            res = 200.0*input.doubleValueOf(i-1)-200.0*input.doubleValueOf(i-2)*input.doubleValueOf(i-2)-400.0*input.doubleValueOf(i-1)*input.doubleValueOf(i)+400.0*Math.pow(input.doubleValueOf(i-1), 3)+2.0*input.doubleValueOf(i-1)-2.0;
        }
        else if (i == n)
        {
            res = 200.0*input.doubleValueOf(n-1)-200.0*input.doubleValueOf(n-2)*input.doubleValueOf(n-2);
        }
