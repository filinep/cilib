 ...
 */
package net.sourceforge.cilib.fss.feeding;
    ...
    public void feed(Fish fish, Topology<Fish> topology, double normValue) {       
        if (normValue != 0) {
            ...
            double newWeight = Math.min(Math.max(
                fish.getMinWeight().getParameter(), fish.getWeight() + normDeltaF),
                fish.getMaxWeight().getParameter()
            );
            
            fish.setDeltaF(normDeltaF);
            fish.setDeltaW(newWeight - fish.getWeight());
